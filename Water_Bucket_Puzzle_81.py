...
GOAL = 4 #the exact amount of water (in litres) to have in a bucket to win
steps = 0 #keep track of how many steps the player makes to solve this
...
#Display the current state of the buckets:
    print()
    print('Try to get ' + str(GOAL) + ' litres of water into one of these')
    print('buckets')

    waterDisplay = []#Contains strings for water or empty space

    #Get the strings for the 8 litre bucket:
    for i in range(1, 9):
    ...
    #Get the strings for the 3 litre bucket:
    for i in range(1, 4):
...
'''.format(*waterDisplay))
    ...
    #Check if the buckets has the goal amount of water:
    ...
    while True: #Keep asking until a valid bucket is entered
    ...
    if move == 'F':
        ...
        while True: #Keep asking until a valid bucket is entered.
            ...
            if dstBucket == 'QUIT' or dstBucket == 'Q':
                print('Thanks for playing!')
                time.sleep(3)
                os._exit(1)
                #sys.exit()
        ...
        # Put the poured water into the other bucket:
